...
leia = users.find(function(element, index, array){
        return element.id === 2;
...
var newReplacement = replacement.replace(pattern, 'Artie');
...
//foreach does not actually return anything unless you code it do so, map cycles thru the array the same way foreach does and returns a new array based on the code you provided. 
...
//filter iterates thru the entire array/object and returns every "true" condition that you defined, find iterates thru the object until it encounters and returns the first true condition it finds. 
